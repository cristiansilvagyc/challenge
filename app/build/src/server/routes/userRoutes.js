"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    function adopt(value) { return value instanceof P ? value : new P(function (resolve) { resolve(value); }); }
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : adopt(result.value).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __importDefault = (this && this.__importDefault) || function (mod) {
    return (mod && mod.__esModule) ? mod : { "default": mod };
};
const express_1 = require("express");
const userController_1 = __importDefault(require("../controller/userController"));
const router = (0, express_1.Router)();
const controller = new userController_1.default();
router.get('/:userId', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { userId } = req.params;
        const resp = yield controller.getUserById(Number(userId));
        res.status(200).json(resp);
    }
    catch (err) {
        next(err);
    }
    ;
}));
router.post('/', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const resp = yield controller.updateUser(req.body);
        res.status(200).json(resp);
    }
    catch (err) {
        next(err);
    }
}));
router.put('/', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const resp = yield controller.addUser(req.body);
        res.status(200).json(resp);
    }
    catch (err) {
        next(err);
    }
}));
router.delete('/:userId', (req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        const { userId } = req.params;
        const resp = yield controller.deleteUser(Number(userId));
        res.status(200).json(resp);
    }
    catch (err) {
        next(err);
    }
}));
module.exports = router;
//# sourceMappingURL=userRoutes.js.map